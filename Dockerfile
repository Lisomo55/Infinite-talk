...
# Show where the base handler lives (visible in build logs)
RUN find / -name "rp_handler.py" -o -name "handler.py" 2>/dev/null | grep -v comfyui | grep -v proc | grep -v sys || true

# Copy to all likely locations
